...
class Generator:
    ...
    def __init__(self, x, y, cont_number, filename):
        borders = [15, 59, 71, 96]
        t, size_continent = 11, 20
        self.array = self.map_generator(x, y, cont_number, filename, borders, t, size_continent)

        names = ['Water_deep.png', 'Water_not_deep.png', 'Plains.png', 'Forests.png', 'Mountains.png',
                 'Snowy_mountains.png']
        images = []
        im = Image
        # Создание массива текстур, для ускорения работы вывода изображений
        for name in names:
            im = Image.open('../ResourcesDirectory/' + name)
            images.append(im)

        self.image_out = Image.new(im.mode, (len(self.array) * 41, len(self.array[0]) * 41))
        for x in range(len(self.array)):
            for y in range(len(self.array[0])):
                self.image_out.paste(images[self.array[x][y]], (x * 41, y * 41))
        self.image_out.save('../ResultsDirectory/' + filename + ".png")

    # Генерация одного континента
    def continent_generate(self, map, center_x, center_y, size_of_chunks_x, size_of_chunks_y, continent_size):
    ...
    def generate_map_of_continents(self, map, size_of_chunks_x, size_of_chunks_y, number_of_chunks_x,
                                   number_of_chunks_y,
        ...
        while numof_generated_ccntinents < number_of_continents:
            ...
            if map.stock[size_of_chunks_x * rand_x][size_of_chunks_y * rand_y].typeres == 0:
                ...
                # Генерируем континент
                self.continent_generate(map, rand_x, rand_y, size_of_chunks_x, size_of_chunks_y, size)
    ...
    def generate_chunk(self, map, center_x, center_y, radius, normal, t):
        counter = 0
    ...
    def generate_landscape(self, map, number_of_chunks_x, number_of_chunks_y, size_of_chunks_x, size_of_chunks_y,
                           normal, t):
        # Идём по всем чанкам
        for i in range(1, number_of_chunks_x + 1):
            for j in range(1, number_of_chunks_y + 1):
                ...
                # Генерируем "чанк"
                self.generate_chunk(map, center_x, center_y, radius, normal, t)

    # Cохранение массива высот в txt файле filename
    def write_map_array(self, stock, filename):
    ...
    # Генерация карты
    def map_generator(self, sizemap_x, sizemap_y, number_of_continents, filename, normal, t, size):
        ...
        self.generate_map_of_continents(map, size_of_chunks_x, size_of_chunks_y, number_of_chunks_x, number_of_chunks_y,
                                        number_of_continents, size)
        ...
        self.generate_landscape(map, number_of_chunks_x, number_of_chunks_y, size_of_chunks_x, size_of_chunks_y, normal,
                                t)
        ...
        self.write_map_array(map.interpolated_map, filename)

        return map.interpolated_map


if __name__ == '__main__':
    x, y, cnt, file = int(input()), int(input()), int(input()), input()
    gen = Generator(x, y, cnt, file)
